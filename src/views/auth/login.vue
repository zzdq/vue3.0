<template>
  <div class="login-container">
    <el-form
      ref="loginForm"
      :model="state.authLoginForm"
      :rules="state.authLoginRules"
      label-width="0px"
      status-icon
      class="hoverable"
    >
      <div>
        <div style="text-align:center; width: 100%;">
          <img
            :src="state.configData.index_logo || '@/assets/logo.png'"
            style="text-align:center; height: 50px; margin-top: 20px;margin-bottom: -20px;"
          >
        </div>
        <h1 class="login-title">{{ state.configData.index_title ? state.configData.index_title : '全生命周期智慧电梯云平台' }}</h1>
      </div>
      <el-form-item
        label
        prop="username"
      >
        <el-input
          v-model.trim="state.authLoginForm.username"
          type="text"
          placeholder="请输入用户名"
          clearable
          required
      ...
      >
        <el-input
          v-model.trim="state.authLoginForm.password"
          type="password"
          placeholder="请输入密码"
          clearable
          required
